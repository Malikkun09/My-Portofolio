/* ========== SMOOTH SCROLL FUNCTION ========== */
    ...
    function initNavbarIndicator(){
      ...
      navLinks.forEach(link => {
        link.addEventListener('click', (e) => {
          ...
          // Skip PPT button and external links
          if(link.classList.contains('ppt-btn') || href.startsWith('http') || href.startsWith('index-PPT')){
    ...
    function initSkillBars(){
      ...
      // Tambahkan style gradient ke semua progress-fill
      const gradientStyle = "linear-gradient(90deg, #ff3f34 0%, #ffb86b 16.66%, #ffd26a 33.33%, #93ff8a 50%, #5bd8ff 66.66%, #a49bff 83.33%, #ff6fd8 100%)";
      skillBars.forEach(bar => {
        bar.style.background = gradientStyle;
        bar.style.animation = "rainbow-pulse 2s ease-in-out infinite";
      });
      ...
      const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
          if(entry.isIntersecting){
            ...
            console.log(`Animating bar to ${percentage}%`, bar);
            
            // Trigger reflow to restart animation
            bar.style.width = '0%';
            void bar.offsetWidth; // Force reflow
            
            // Set width after reflow
            setTimeout(() => {
      ...
      skillBars.forEach(bar => {
        console.log('Observing bar:', bar);
        observer.observe(bar);
      });
      
      // Fallback: Jika tidak ada intersection observer, langsung animate setelah 1 detik
      setTimeout(() => {
        skillBars.forEach(bar => {
          if(bar.style.width === '0%' || !bar.style.width){
            const percentage = bar.getAttribute('data-skill');
            bar.style.width = percentage + '%';
            console.log(`Fallback: Set width to ${percentage}%`);
          }
        });
      }, 1000);
    }
