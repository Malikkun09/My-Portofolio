/* =========================
    ...
    // GANTI: bulk generator -> kosongkan daftar (kamu simpan projek statis di HTML)
    const sampleProjects = [];
    const sampleWIP = [];

    // helper sederhana cek duplikat (by id)
    function hasProject(id, list){
      return Array.isArray(list) && list.some(p => String(p.id) === String(id));
    }

    // helper buat object project (opsional)
    function createProject({ id, title = 'Untitled', desc = '', img = '', category = 'web' } = {}){
      if (!id) throw new Error('createProject: id is required');
      return { id, title, desc, img, category };
    }

    // tambahkan satu project (JS) - menambah ke array & append ke DOM
    function addProject(project){
      if (!project || !project.id) throw new Error('addProject: project.id required');
      if (hasProject(project.id, sampleProjects) || document.querySelector(`#projectsGrid [data-id="${project.id}"]`)) {
        console.warn('Project id already exists:', project.id); return;
      }
      sampleProjects.push(project);
      const c = makeCard(project); // makeCard dideklarasikan di bawah (hoisted)
      projectsGrid.appendChild(c);
      obs.observe(c);
    }

    // tambahkan satu WIP (JS)
    function addWIP(project){
      if (!project || !project.id) throw new Error('addWIP: project.id required');
      if (hasProject(project.id, sampleWIP) || document.querySelector(`#workInProgressGrid [data-id="${project.id}"]`)) {
        console.warn('WIP id already exists:', project.id); return;
      }
      sampleWIP.push(project);
      const c = makeCard(project);
      workInProgressGrid.appendChild(c);
      obs.observe(c);
    }

    // Contoh penggunaan (uncomment untuk menambah via JS):
    // addProject(createProject({ id: 'p3', title: 'Contoh JS Project', desc: 'Ditambahkan satu-per-satu via addProject', img: 'https://picsum.photos/seed/p3/600/400', category: 'web' }));
    // addWIP(createProject({ id: 'w3', title: 'Contoh WIP', desc: 'WIP ditambahkan melalui addWIP', img: 'https://picsum.photos/seed/w3/600/400', category: 'game' }));
    ...
    const otherSitesData = [
      {name:'Blog Foto', img:'img/blogfoto.jpeg', href:'#'},
      {name:'Hiburan', img:'img/hiburan.jpeg', href:'#'},
      {name:'Downloader', img:'img/downloader2.png', href:'#'},
      {name:'Another Portofolio', img:'https://picsum.photos/seed/site4/200/200', href:'#'}
    ];
    ...
    function renderProjects(filter='all'){
      // Jika tidak ada data JS, jangan hapus kartu statis di HTML — cukup observe elemen yang ada
      if (!sampleProjects || sampleProjects.length === 0) {
        Array.from(projectsGrid.children).forEach(c => { obs.observe(c); });
        return;
      }
    ...
    function renderWIP(){
      if (!sampleWIP || sampleWIP.length === 0) {
        Array.from(workInProgressGrid.children).forEach(c => { obs.observe(c); });
        return;
      }
...
// fallback ke nomor WA langsung sehingga .env tidak lagi wajib
const waLink = (window.__ENV && window.__ENV.NOMOR_MALIK) ? window.__ENV.NOMOR_MALIK : 'https://wa.me/628891983723';

// Navbar link
const contactLink = document.getElementById("contactLink");
if (contactLink) {
  contactLink.href = waLink;
